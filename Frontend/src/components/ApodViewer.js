...
const ApodViewer = () => {
  ...
  const fetchApod = async (date) => {
    try {
      ...
      const endpoint = date ? `/api/nasa/apod?date=${date}` : `/api/nasa/apod`;
  ...
  const fetchRandomApod = async () => {
    try {
      ...
      setError(null);
      setImageLoaded(false);
      const response = await axios.get('/api/nasa/apod?count=1');
  ...
  const fetchApodHistory = async () => {
    try {
      ...
      const response = await axios.get(
        `/api/nasa/apod?start_date=${start.format('YYYY-MM-DD')}&end_date=${end.format('YYYY-MM-DD')}`
      );
